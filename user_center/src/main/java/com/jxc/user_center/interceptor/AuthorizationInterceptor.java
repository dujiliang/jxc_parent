...
public class AuthorizationInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(handler instanceof HandlerMethod) {
        	...
        	driverOrOwner = ((HandlerMethod) handler).getMethodAnnotation(DriverOrOwner.class);
        	
        	
        	if(loginAnnotation == null && driverAnnotation == null && ownerAnnotation == null && tenantryAnnotation == null && allPermission == null && driverOrOwner == null){
        		return true;
        	}else{
        		return dealAuthentication(request, response, loginAnnotation, driverAnnotation, ownerAnnotation,
						tenantryAnnotation, tenantryAdminAnnotation , childAnnotation , allPermission,driverOrOwner);
        	}
	...
	private boolean dealAuthentication(HttpServletRequest request, HttpServletResponse response, Login loginAnnotation,
			Driver driverAnnotation, Owner ownerAnnotation, Tenantry tenantryAnnotation, TenantryAdmin tenantryAdminAnnotation, Child childAnnotation , AllPermission allPermission ,DriverOrOwner driverOrOwner) {
		//从header中获取token
...
//			    	Map<String,Object> data = (Map<String, Object>) js.get("data");
			    	...
			    	try {
				    	...
				    	if(userId != null && roleType != null ){     
				    		...
				    		if (driverOrOwner != null) {
				    			
				    			if(Integer.valueOf(roleType) == RoleTyleConstants.DRIVER_ROLE_TYPE  || Integer.valueOf(roleType) == RoleTyleConstants.OWNER_ROLE_TYPE ){
				    				request.setAttribute(SessionConstants.USER_ID_DRIVER_OR_OWNER, userId);
				    				request.setAttribute(SessionConstants.ROLE_TYPE_DRIVER_OR_OWNER, roleType);
				    			}else{
					    			Io.preForOut(R.error(CommonErrCodeConstants.AUTHENTICATION_TOKEN_NOT_EQUAL,"token权限与当前不一致------机主或者司机"),response);
		    			            return false;
					    		}
